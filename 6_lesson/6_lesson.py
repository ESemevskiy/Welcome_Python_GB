#1. Создать класс TrafficLight (светофор) и определить у
...
class TrafficLight:
    ...
    def running(self):
        time_iter = cycle(self.__time)
        color_iter = cycle(self.__color)
        while True:
            print(f'Светофор переключается в {next(color_iter, 1)}')
            time.sleep(next(time_iter, 1))         
...
class Road:
    ...
    def mass(self):
        print(f'Масса асфальта для покрытия дороги равна {self._length * self._width * 25 * 5 /1000:.2f}т')
...
#manager = Position('Иван', 'Иванов', 'Менеджер', {'wage': 100000, 'bonus': 20000})
...
police_car = PoliceCar(100, 'red', 'police', True)

work_car = WorkCar(100, 'red', 'work', False)

town_car = TownCar(120, 'blue', 'town', False)
